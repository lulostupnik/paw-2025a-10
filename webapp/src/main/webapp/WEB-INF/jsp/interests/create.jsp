...
<html>
<head>
  <title><spring:message code="createInterest.title"/></title>
  <!-- Include custom CSS -->
...
<div class="auth-container">
  <div class="auth-card">
    <div class="auth-header">
      ...
      <h1 class="auth-title"><spring:message code="createInterest.title" text="Create Interest"/></h1>
      <p class="auth-subtitle"><spring:message code="createInterest.subtitle" text="Add a new interest to the system"/></p>
    </div>

    <c:url var="createInterestUrl" value="/interests/create"/>
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    interestForm.addEventListener("submit", (event) => {
      ...
      if (!nameEsInput.value.trim()) {
        ...
        const existingError = nameEsInput.parentNode.querySelector(".error-message");
        if (!existingError) {
          nameEsInput.parentNode.appendChild(errorElement);
        }

        nameEsInput.classList.add("error");
        isValid = false;
      } else {
        nameEsInput.classList.remove("error");
        const existingError = nameEsInput.parentNode.querySelector(".error-message");
        if (existingError) {
          existingError.remove();
        }
